...
% create folders
cd ../
if ~exist('3MATLABsavefiles\3MCMC\WM793b\2Hypoxia', 'dir')
    mkdir(['3MATLABsavefiles\3MCMC\WM793b\2Hypoxia']);
end
if ~exist('4Figures\3MCMC\WM793b\2Hypoxia', 'dir')
    mkdir(['4Figures\3MCMC\WM793b\2Hypoxia']);
end

filepath_save = [newdir '\3MATLABsavefiles\3MCMC\WM793b\2Hypoxia\'];
filepath_save_figs =  [newdir '\4Figures\3MCMC\WM793b\2Hypoxia\'];

simulation_id = 'WM793b_02_HypoxiaGreenspan';
...
% remove outliers
WM793b_outer_tmp = load([filepath_load 'WM793b_confocal_scenario_2_outer.mat']);
WM793b_necrotic_tmp = load([filepath_load 'WM793b_confocal_scenario_2_necrotic.mat']);
WM793b_inhibited_tmp = load([filepath_load 'WM793b_confocal_scenario_2_inhibited.mat']);
...
num_spheroids = length(WM793b_outer.Radius);

Rcrit_estimate_per_spheroid = [];
Rcrit_estimate_counter = 1;
for i=1:num_spheroids
    if (WM793b_outer_tmp.Outlier(i)==0) && (WM793b_necrotic_tmp.Outlier(i)==0) % include if measurement is not an outlier
        OuterRadius_this_loop = WM793b_outer_tmp.Radius(i);
        NecroticRadius_this_loop = WM793b_necrotic_tmp.Radius(i);
...
for i=1:num_spheroids
    ...
    if (WM793b_outer_tmp.Outlier(i)==0) && (WM793b_necrotic_tmp.Outlier(i)==0) && (WM793b_inhibited_tmp.Outlier(i)==0) % include if measurement is not an outlier

        OuterRadius_this_loop = WM793b_outer_tmp.Radius(i);
        NecroticRadius_this_loop = WM793b_necrotic_tmp.Radius(i);
        InhibitedRadius_this_loop = WM793b_inhibited_tmp.Radius(i);
...
data.xdata = [WM793b_outer.Day;WM793b_necrotic.Day;WM793b_inhibited.Day]-day_shift; % Days
...
close all
save([filepath_save 'MCMC_WM793b_02_HypoxiaGreenspan' '.mat'],'-v7.3');
...
if include_plots == 1
    ...
    load([filepath_save 'MCMC_WM793b_02_HypoxiaGreenspan' '.mat']);
    ...
    fig_boxchart_predinterval = openfig([newdir '\4Figures\1FirstAnalysisExperimentalData\WM793b\Scenario_2_OIN_1.fig']);
