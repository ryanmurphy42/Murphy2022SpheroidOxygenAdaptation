...
% 2.1) Estimate initial necrotic radius, Rn(0)
...
% create folders
cd ../
if ~exist('3MATLABsavefiles\3MCMC\WM164\12ReoxygenationAllD4', 'dir')
    mkdir(['3MATLABsavefiles\3MCMC\WM164\12ReoxygenationAllD4']);
end
if ~exist('4Figures\3MCMC\WM164\12ReoxygenationAllD4', 'dir')
    mkdir(['4Figures\3MCMC\WM164\12ReoxygenationAllD4']);
end


filepath_save = [newdir '\3MATLABsavefiles\3MCMC\WM164\12ReoxygenationAllD4\'];
filepath_save_figs =  [newdir '\4Figures\3MCMC\WM164\12ReoxygenationAllD4\'];

simulation_id = 'WM164_12_ReoxygenationAllD4';
...
% remove outliers
WM164_outer_tmp = load([filepath_load 'WM164_confocal_scenario_5_outer.mat']);
WM164_necrotic_tmp = load([filepath_load 'WM164_confocal_scenario_5_necrotic.mat']);
WM164_inhibited_tmp = load([filepath_load 'WM164_confocal_scenario_5_inhibited.mat']);
...
filepath_load_normoxia = [newdir '\3MATLABsavefiles\3MCMC\WM164\1Normoxia\'];
filepath_load_hypoxia = [newdir '\3MATLABsavefiles\3MCMC\WM164\2Hypoxia\'];
filepath_load_alpha = [newdir '\3MATLABsavefiles\3MCMC\WM164\10ReoxygenationAlpha_D4\'];
filepath_load_Rd  = [newdir '\3MATLABsavefiles\3MCMC\WM164\11ReoxygenationRd_D4\'];

load_normoxia = load([filepath_load_normoxia 'MCMC_WM164_01_NormoxiaGreenspan.mat'],"-mat",'params_withnormalfit');
load_hypoxia = load([filepath_load_hypoxia 'MCMC_WM164_02_HypoxiaGreenspan.mat'],"-mat",'params_withnormalfit');
load_alpha = load([filepath_load_alpha 'WM164_10_ReoxygenationAlpha_D4.mat'],"-mat",'params_withnormalfit');
load_Rd = load([filepath_load_Rd 'WM164_11_ReoxygenationRd_D4.mat'],"-mat",'params_withnormalfit');
...
% select all outer radii data on the first day (and exclude outliers)
NecroticRadius_initial = WM164_necrotic.Radius(logical((WM164_necrotic.Outlier==0).*(WM164_necrotic.Day==day_min)));

% fit a normal distribution to data
NecroticRadiusInitial_normaldistribution_fit = fitdist(NecroticRadius_initial,'Normal');

% output mean and standard deviation
NecroticRadiusInitial_normaldistribution_fit.mu;
NecroticRadiusInitial_normaldistribution_fit.sigma;
...
% theta(12) = tau_lambda;
% theta(13) = lambdatilde;
% theta(14) = tau_lambdatilde;
% theta(15) = outer_radii_at_ts;
% theta(16) = nu;
% theta(17) = necrotic_radii_at_ts;


data.xdata = [WM164_outer.Day;WM164_necrotic.Day;WM164_inhibited.Day]; % Days
...
p_infinity = 21; % [percent]

k_unconverted = 2*10^(-9); % [m^2 s^-1]
k = k_unconverted; %*(1/(1*10^(-6))^2)*(1./(60*60*24)); % [micrometre^2 day^(-1)]
t_s=4;

modelfun = @(x,theta) function_reoxygenation_1simulationbiphase(theta,...
    k,...
    p_infinity,...
    Omega,...
    t_s,...
...
theta_first_guess = [alpha_normoxia_first_guess_from_Rc,...% theta(1) alpha_normoxia
    alpha_hypoxia_first_guess_from_Rc,...% theta(2) = %alpha_hypoxia
    load_alpha.params_withnormalfit{3}{7},...% theta(3) = %tau_alpha
    load_Rd.params_withnormalfit{1}{7},... %Rd_normoxia
	load_Rd.params_withnormalfit{2}{7},... %Rd_hypoxia
    load_Rd.params_withnormalfit{3}{7},...% theta(6) = %tau_Rd
    load_normoxia.params_withnormalfit{3}{7},...% theta(7) = %s_normoxia
    load_hypoxia.params_withnormalfit{3}{7},...% theta(8) = %s_hypoxia
    1,...% theta(9) = %tau_s
    load_normoxia.params_withnormalfit{2}{7},...% theta(10) = %lambda_normoxia
    load_hypoxia.params_withnormalfit{2}{7},...% theta(11) = %lambda_hypoxia
    1,...% theta(12) = %tau_lambda
    0.01,...% theta(13) = %lambdatilde
    1,...% theta(14) = %tau_lambdatilde
    OuterRadiusInitial_normaldistribution_fit.mu,...% theta(15) = %outer_radii_at_ts
    0.01,...% theta(16) = % nu
    NecroticRadiusInitial_normaldistribution_fit.mu]; % theta(17) = necrotic_radii_at_ts
...
p_lower_bounds = [max(0.01,alpha_normoxia_lowerbound_from_Rc),...% theta(1) alpha_normoxia
    ...
    0.001,...% theta(13) = %lambdatilde
    0.1,...% theta(14) = %tau_lambdatilde
    OuterRadiusInitial_normaldistribution_fit.mu-3*OuterRadiusInitial_normaldistribution_fit.sigma,...% theta(15) = %outer_radii_at_t0
    0,...% theta(16) = % nu
    max(0,NecroticRadiusInitial_normaldistribution_fit.mu-5*NecroticRadiusInitial_normaldistribution_fit.sigma)];  % theta(17) = necrotic_radii_at_t0
p_upper_bounds = [min(50,alpha_normoxia_upperbound_from_Rc),...% theta(1) alpha_normoxia
    ...
    10,...% theta(13) = %lambdatilde
    10,...% theta(14) = %tau_lambdatilde
    OuterRadiusInitial_normaldistribution_fit.mu+3*OuterRadiusInitial_normaldistribution_fit.sigma,...% theta(15) = %outer_radii_at_t0
    1,... % theta(16) = % nu
    max(1,NecroticRadiusInitial_normaldistribution_fit.mu+5*NecroticRadiusInitial_normaldistribution_fit.sigma)];% theta(17) = necrotic_radii_at_t0
...
[ydata_mse] = function_reoxygenation_1simulationbiphase(p_mle,...
    k,...
    p_infinity,...
    Omega,...
    t_s,...
...
params = {
    ...
    {'lambdatilde',p_mle(13),p_lower_bounds(13), p_upper_bounds(13)}
    {'tau_lambdatilde',p_mle(14),p_lower_bounds(14), p_upper_bounds(14)}
    {'outerradii_at_t0',p_mle(15),p_lower_bounds(15), p_upper_bounds(15),p_upper_bounds(15),OuterRadiusInitial_normaldistribution_fit.mu,2*OuterRadiusInitial_normaldistribution_fit.sigma}
    {'nu',p_mle(16),p_lower_bounds(16), p_upper_bounds(16)}
    {'necroticradii_at_t0',p_mle(17),p_lower_bounds(17), p_upper_bounds(17),NecroticRadiusInitial_normaldistribution_fit.mu,max(1,2*NecroticRadiusInitial_normaldistribution_fit.sigma)}
	};	
...
modelfun_prediction_interval = @(x,theta) function_reoxygenation_1simulationbiphase(theta,...
    k,...
    p_infinity,...
    Omega,...
    t_s,...
...
close all
save([filepath_save 'MCMC_WM164_12_ReoxygenationAllD4' '.mat'],'-v7.3');
...
if include_plots == 1
    ...
    load([filepath_save 'MCMC_WM164_08_ReoxygenationAllD4' '.mat']);
    ...
    [ydata_globalmin] = function_reoxygenation_1simulationbiphase(p_mle,...
        k,...
        p_infinity,...
        Omega,...
        t_s,...
    ...
    [ydata_meanchain] = function_reoxygenation_1simulationbiphase(mean(chain),...
        k,...
        p_infinity,...
        Omega,...
        t_s,...
    ...
    fig_boxchart_predinterval = openfig([newdir '\4Figures\1FirstAnalysisExperimentalData\WM164\Scenario_5_OIN_1.fig']);
