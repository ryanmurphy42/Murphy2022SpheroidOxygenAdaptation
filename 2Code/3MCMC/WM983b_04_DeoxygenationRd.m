...
% create folders
cd ../
if ~exist('3MATLABsavefiles\3MCMC\WM983b\4DeoxygenationRd', 'dir')
    mkdir(['3MATLABsavefiles\3MCMC\WM983b\4DeoxygenationRd']);
end
if ~exist('4Figures\3MCMC\WM983b\4DeoxygenationRd', 'dir')
    mkdir(['4Figures\3MCMC\WM983b\4DeoxygenationRd']);
end

filepath_save = [newdir '\3MATLABsavefiles\3MCMC\WM983b\4DeoxygenationRd\'];
filepath_save_figs =  [newdir '\4Figures\3MCMC\WM983b\4DeoxygenationRd\'];

simulation_id = 'WM983b_04_DeoxygenationRd';
...
deoxygenation_Rd_data= load([filepath_load 'WM983b_oxygenwasteanalysis_deoxy.mat']);

data.xdata = deoxygenation_Rd_data.confocal_WM983b.AllData.Day(deoxygenation_Rd_data.Rd_vector>=0); % select all data and filter to valid data (i.e when Rd >=0)
data.ydata = deoxygenation_Rd_data.Rd_vector(deoxygenation_Rd_data.Rd_vector>=0);
...
modelfun = @(x,theta) theta(2) + (theta(1)-theta(2)).*exp(-(1/theta(3))*(x-t_s));
ssfun    = @(theta,data) sum((data.ydata-modelfun(data.xdata,theta)).^2);

%% 2.1) Global minimisation for first guess


theta_first_guess =  [Rd_initial_time_normaldistribution_fit.mu; % Rd_normoxia
    Rd_final_time_normaldistribution_fit.mu;  % Rd_hypoxia
    0.5]; % tau_Rd

fun_likelihood = @(p) ssfun(p,data);

p_first_guess = theta_first_guess;
p_lower_bounds =  [max(0,Rd_initial_time_normaldistribution_fit.mu - 5*Rd_initial_time_normaldistribution_fit.sigma); % Rd_normoxia
    max(0,Rd_final_time_normaldistribution_fit.mu - 5*Rd_final_time_normaldistribution_fit.sigma);  % Rd_hypoxia
    0.001]; % tau_Rd
p_upper_bounds =  [Rd_initial_time_normaldistribution_fit.mu + 5*Rd_initial_time_normaldistribution_fit.sigma; % Rd_normoxia
    Rd_final_time_normaldistribution_fit.mu + 5*Rd_final_time_normaldistribution_fit.sigma;  % Rd_hypoxia
    100]; % tau_Rd
...
params = {
    {'Rd normoxia', p_mle(1), p_lower_bounds(1),p_upper_bounds(1),Rd_initial_time_normaldistribution_fit.mu,3*Rd_initial_time_normaldistribution_fit.sigma}
    {'Rd hypoxia', p_mle(2), p_lower_bounds(2),p_upper_bounds(2),Rd_final_time_normaldistribution_fit.mu,3*Rd_final_time_normaldistribution_fit.sigma}
...
save([filepath_save 'WM983b_04_DeoxygenationRd' '.mat'],'-v7.3');
