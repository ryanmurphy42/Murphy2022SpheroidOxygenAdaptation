...
% 1) Load data
% 1.1) Add synthetic data at t=2.5 [days]
...
% create folders
cd ../
if ~exist('3MATLABsavefiles\3MCMC\WM983b\6DeoxygenationAllD2D3interp', 'dir')
    mkdir(['3MATLABsavefiles\3MCMC\WM983b\6DeoxygenationAllD2D3interp']);
end
if ~exist('4Figures\3MCMC\WM983b\6DeoxygenationAllD2D3interp', 'dir')
    mkdir(['4Figures\3MCMC\WM983b\6DeoxygenationAllD2D3interp']);
end


filepath_save = [newdir '\3MATLABsavefiles\3MCMC\WM983b\6DeoxygenationAllD2D3interp\'];
filepath_save_figs =  [newdir '\4Figures\3MCMC\WM983b\6DeoxygenationAllD2D3interp\'];

simulation_id = 'WM983b_06_DeoxygenationAllD2D3interp';
...
% open data from 1Data folder
filepath_load = [newdir '\3MATLABsavefiles\1FirstAnalysisExperimentalData\WM983b\'];

% remove outliers
WM983b_outer_tmp = load([filepath_load 'WM983b_confocal_scenario_3_outer.mat']);
WM983b_necrotic_tmp = load([filepath_load 'WM983b_confocal_scenario_3_necrotic.mat']);
WM983b_inhibited_tmp = load([filepath_load 'WM983b_confocal_scenario_3_inhibited.mat']);

WM983b_outer.Day = WM983b_outer_tmp.Day(WM983b_outer_tmp.Outlier == 0);
WM983b_outer.Radius = WM983b_outer_tmp.Radius(WM983b_outer_tmp.Outlier == 0);
WM983b_outer.Outlier = WM983b_outer_tmp.Outlier(WM983b_outer_tmp.Outlier == 0);

WM983b_necrotic.Day = WM983b_necrotic_tmp.Day(WM983b_necrotic_tmp.Outlier == 0);
WM983b_necrotic.Radius = WM983b_necrotic_tmp.Radius(WM983b_necrotic_tmp.Outlier == 0);
WM983b_necrotic.Outlier = WM983b_necrotic_tmp.Outlier(WM983b_necrotic_tmp.Outlier == 0);

WM983b_inhibited.Day = WM983b_inhibited_tmp.Day(WM983b_inhibited_tmp.Outlier == 0);
WM983b_inhibited.Radius = WM983b_inhibited_tmp.Radius(WM983b_inhibited_tmp.Outlier == 0);
WM983b_inhibited.Outlier = WM983b_inhibited_tmp.Outlier(WM983b_inhibited_tmp.Outlier == 0);

% load data from normoxia, hypoxia, and deoxygenation alpha and Rd .mat
% files to inform parameter estimation bounds

filepath_load_normoxia = [newdir '\3MATLABsavefiles\3MCMC\WM983b\1Normoxia\'];
filepath_load_hypoxia = [newdir '\3MATLABsavefiles\3MCMC\WM983b\2Hypoxia\'];
filepath_load_alpha = [newdir '\3MATLABsavefiles\3MCMC\WM983b\3DeoxygenationAlpha\'];
filepath_load_Rd  = [newdir '\3MATLABsavefiles\3MCMC\WM983b\4DeoxygenationRd\'];

load_normoxia = load([filepath_load_normoxia 'MCMC_WM983b_01_NormoxiaGreenspan.mat'],"-mat",'params_withnormalfit');
load_hypoxia = load([filepath_load_hypoxia 'MCMC_WM983b_02_HypoxiaGreenspan.mat'],"-mat",'params_withnormalfit');
load_alpha = load([filepath_load_alpha 'WM983b_03_DeoxygenationAlpha.mat'],"-mat",'params_withnormalfit');
load_Rd = load([filepath_load_Rd 'WM983b_04_DeoxygenationRd.mat'],"-mat",'params_withnormalfit');


%% 1.1) Add synthetic data at t=2.5 [days]


% Estimate the pooled standard deviation for outer, necrotic and inhibited
% for Ro - estimate mean and standard deviation per timepoint
unique_hours_Ro = sort(unique(WM983b_outer.Day));
mean_at_timepoint_Ro = zeros(length(unique_hours_Ro),1);
standard_deviation_at_timepoint_Ro = zeros(length(unique_hours_Ro),1);
for u1_Ro=1:length(unique_hours_Ro)
    mean_at_timepoint_Ro(u1_Ro) = mean(WM983b_outer.Radius(WM983b_outer.Day == unique_hours_Ro(u1_Ro)));
    standard_deviation_at_timepoint_Ro(u1_Ro) = std(WM983b_outer.Radius(WM983b_outer.Day == unique_hours_Ro(u1_Ro)));
end

% for Ro - estimate pooled standard deivation across all
sum_for_pooled_standarddeviation_Ro = 0;
for u2_Ro=1:length(WM983b_outer.Day)
    sum_for_pooled_standarddeviation_Ro = sum_for_pooled_standarddeviation_Ro ...
        + (WM983b_outer.Radius(u2_Ro) - mean_at_timepoint_Ro(unique_hours_Ro==WM983b_outer.Day(u2_Ro)) ).^2;
end
pooled_standard_deviation_Ro = sqrt((1./(length(WM983b_outer.Day)-1)).*sum_for_pooled_standarddeviation_Ro);

% for Rn - estimate mean and standard deviation per timepoint
unique_hours_Rn = sort(unique(WM983b_necrotic.Day));
mean_at_timepoint_Rn = zeros(length(unique_hours_Rn),1);
standard_deviation_at_timepoint_Rn = zeros(length(unique_hours_Rn),1);
for u1_Rn=1:length(unique_hours_Rn)
    mean_at_timepoint_Rn(u1_Rn) = mean(WM983b_necrotic.Radius(WM983b_necrotic.Day == unique_hours_Rn(u1_Rn)));
    standard_deviation_at_timepoint_Rn(u1_Rn) = std(WM983b_necrotic.Radius(WM983b_necrotic.Day == unique_hours_Rn(u1_Rn)));
end

% for Rn - estimate pooled standard deivation across all
sum_for_pooled_standarddeviation_Rn = 0;
for u2_Rn=1:length(WM983b_necrotic.Day)
    sum_for_pooled_standarddeviation_Rn = sum_for_pooled_standarddeviation_Rn ...
        + (WM983b_necrotic.Radius(u2_Rn) - mean_at_timepoint_Rn(unique_hours_Rn==WM983b_necrotic.Day(u2_Rn)) ).^2;
end
pooled_standard_deviation_Rn = sqrt((1./(length(WM983b_necrotic.Day)-1)).*sum_for_pooled_standarddeviation_Rn);

% for Ri - estimate mean and standard deviation per timepoint
unique_hours_Ri = sort(unique(WM983b_inhibited.Day));
mean_at_timepoint_Ri = zeros(length(unique_hours_Ri),1);
standard_deviation_at_timepoint_Ri = zeros(length(unique_hours_Ri),1);
for u1_Ri=1:length(unique_hours_Ri)
    mean_at_timepoint_Ri(u1_Ri) = mean(WM983b_inhibited.Radius(WM983b_inhibited.Day == unique_hours_Ri(u1_Ri)));
    standard_deviation_at_timepoint_Ri(u1_Ri) = std(WM983b_inhibited.Radius(WM983b_inhibited.Day == unique_hours_Ri(u1_Ri)));
end

% for Ri - estimate pooled standard deivation across all
sum_for_pooled_standarddeviation_Ri = 0;
for u2_Ri=1:length(WM983b_inhibited.Day)
    sum_for_pooled_standarddeviation_Ri = sum_for_pooled_standarddeviation_Ri ...
        + (WM983b_inhibited.Radius(u2_Ri) - mean_at_timepoint_Ri(unique_hours_Ri==WM983b_inhibited.Day(u2_Ri)) ).^2;
end
pooled_standard_deviation_Ri = sqrt((1./(length(WM983b_inhibited.Day)-1)).*sum_for_pooled_standarddeviation_Ri);

sum_for_pooled_standarddeviation_all = sqrt((1./(length(WM983b_outer.Day) + length(WM983b_necrotic.Day) + length(WM983b_inhibited.Day)-1)).*(sum_for_pooled_standarddeviation_Ro + sum_for_pooled_standarddeviation_Rn + sum_for_pooled_standarddeviation_Ri));


% Interpolate D2 and D3 measurements (and add noise)

% calculate mean at D2 and D3 and choose midpoint - linear interpolation
t_interp = 2.5;
t_interp_lower_bound = 2;
t_interp_upper_bound = 3;
Ro_interp = mean_at_timepoint_Ro(unique_hours_Ro == t_interp_lower_bound) + (t_interp_upper_bound - t_interp).*(mean_at_timepoint_Ro(unique_hours_Ro == t_interp_upper_bound) - mean_at_timepoint_Ro(unique_hours_Ro == t_interp_lower_bound));
Rn_interp = mean_at_timepoint_Rn(unique_hours_Rn == t_interp_lower_bound) + (t_interp_upper_bound - t_interp).*(mean_at_timepoint_Rn(unique_hours_Rn == t_interp_upper_bound) - mean_at_timepoint_Rn(unique_hours_Rn == t_interp_lower_bound));
Ri_interp = mean_at_timepoint_Ri(unique_hours_Ri == t_interp_lower_bound) + (t_interp_upper_bound - t_interp).*(mean_at_timepoint_Ri(unique_hours_Ri == t_interp_upper_bound) - mean_at_timepoint_Ri(unique_hours_Ri == t_interp_lower_bound));


% add noise based on pooled_standard_deviation_Ro, pooled_standard_deviation_Rn, pooled_standard_deviation_Ri
rng(14,'twister') % for reproducibility

% assume 8 measurements of each type
num_measurements_at_interp = 8;

% add to
WM983b_outer.Day = [WM983b_outer.Day; t_interp.*ones(num_measurements_at_interp,1)];
WM983b_outer.Radius = [WM983b_outer.Radius;Ro_interp*ones(num_measurements_at_interp,1) + pooled_standard_deviation_Ro*randn(num_measurements_at_interp,1)] ;
WM983b_outer.Outlier = [WM983b_outer.Outlier;zeros(num_measurements_at_interp,1)];

WM983b_necrotic.Day = [WM983b_necrotic.Day; t_interp.*ones(num_measurements_at_interp,1)];
WM983b_necrotic.Radius =[WM983b_necrotic.Radius;Rn_interp*ones(num_measurements_at_interp,1) + pooled_standard_deviation_Rn*randn(num_measurements_at_interp,1)] ;
WM983b_necrotic.Outlier = [WM983b_necrotic.Outlier;zeros(num_measurements_at_interp,1)];

WM983b_inhibited.Day =  [WM983b_inhibited.Day; t_interp.*ones(num_measurements_at_interp,1)];
WM983b_inhibited.Radius = [WM983b_inhibited.Radius;Ri_interp*ones(num_measurements_at_interp,1) + pooled_standard_deviation_Ri*randn(num_measurements_at_interp,1)] ;
WM983b_inhibited.Outlier = [WM983b_inhibited.Outlier ;zeros(num_measurements_at_interp,1)];


%% 2) Estimate initial outer radius, Ro(0)

% obtain first day
day_min = min(WM983b_outer.Day);

% select all outer radii data on the first day (and exclude outliers)
OuterRadius_initial = WM983b_outer.Radius(logical((WM983b_outer.Outlier==0).*(WM983b_outer.Day==day_min)));
...
data.xdata = [WM983b_outer.Day;WM983b_necrotic.Day;WM983b_inhibited.Day]; % Days
data.ydata = [WM983b_outer.Radius;WM983b_necrotic.Radius;WM983b_inhibited.Radius]; % Outer, Necrotic, Inhibited
variable_id_vec = [ones(length(WM983b_outer.Day),1);2*ones(length(WM983b_necrotic.Day),1);3*ones(length(WM983b_inhibited.Day),1) ];
...
% first guess for global minimisations
...
p_upper_bounds = [min(50,alpha_normoxia_upperbound_from_Rc),...% theta(1) alpha_normoxia
    min(50,alpha_hypoxia_upperbound_from_Rc),...% theta(2) = %alpha_hypoxia
...
x_data_outer_pos = 1:length(WM983b_outer.Day);
x_data_necrotic_pos = (length(WM983b_outer.Day)+1):(length(WM983b_outer.Day)+length(WM983b_necrotic.Day));
x_data_inhibited_pos = (length(WM983b_outer.Day)+length(WM983b_necrotic.Day)+1):(length(WM983b_outer.Day)+length(WM983b_necrotic.Day)+length(WM983b_inhibited.Day));
...
close all
save([filepath_save 'MCMC_WM983b_06_DeoxygenationAllD2D3interp' '.mat'],'-v7.3');
...
if include_plots == 1
    ...
    load([filepath_save 'MCMC_WM983b_06_DeoxygenationAllD2D3interp' '.mat']);
    ...
    fig_boxchart_predinterval = openfig([newdir '\4Figures\1FirstAnalysisExperimentalData\WM983b\Scenario_3_OIN_1.fig']);
