...
% create folders
cd ../
if ~exist('3MATLABsavefiles\3MCMC\WM793b\11ReoxygenationRd_D4', 'dir')
    mkdir(['3MATLABsavefiles\3MCMC\WM793b\11ReoxygenationRd_D4']);
end
if ~exist('4Figures\3MCMC\WM793b\11ReoxygenationRd_D4', 'dir')
    mkdir(['4Figures\3MCMC\WM793b\11ReoxygenationRd_D4']);
end

filepath_save = [newdir '\3MATLABsavefiles\3MCMC\WM793b\11ReoxygenationRd_D4\'];
filepath_save_figs =  [newdir '\4Figures\3MCMC\WM793b\11ReoxygenationRd_D4\'];

simulation_id = 'WM793b_11_ReoxygenationRd_D4';
...
deoxygenation_Rd_data= load([filepath_load 'WM793b_oxygenwasteanalysis_reoxyD4.mat']);
...
Rd_initial_time = data.ydata(logical(data.xdata<=t_s));
...
t_s = 4; % time when switches from normoxia to hypoxia

% theta(1) %Rd_normoxia,...
% theta(2) %Rd_hypoxia,...
% theta(3) %tau_Rd

modelfun = @(x,theta) theta(2).*(x<= t_s) + (theta(1) + (theta(2)-theta(1)).*exp(-(1/theta(3))*(x-t_s))).*(x>t_s);
...
save([filepath_save 'WM793b_11_ReoxygenationRd_D4' '.mat'],'-v7.3');
