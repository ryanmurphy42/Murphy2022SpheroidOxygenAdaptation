...
% create folders
cd ../
if ~exist('3MATLABsavefiles\3MCMC\WM164\9ReoxygenationAll', 'dir')
    mkdir(['3MATLABsavefiles\3MCMC\WM164\9ReoxygenationAll']);
end
if ~exist('4Figures\3MCMC\WM164\9ReoxygenationAll', 'dir')
    mkdir(['4Figures\3MCMC\WM164\9ReoxygenationAll']);
end


filepath_save = [newdir '\3MATLABsavefiles\3MCMC\WM164\9ReoxygenationAll\'];
filepath_save_figs =  [newdir '\4Figures\3MCMC\WM164\9ReoxygenationAll\'];

simulation_id = 'WM164_09_ReoxygenationAll';
...
% remove outliers
WM164_outer_tmp = load([filepath_load 'WM164_confocal_scenario_4_outer.mat']);
WM164_necrotic_tmp = load([filepath_load 'WM164_confocal_scenario_4_necrotic.mat']);
WM164_inhibited_tmp = load([filepath_load 'WM164_confocal_scenario_4_inhibited.mat']);
...
filepath_load_alpha = [newdir '\3MATLABsavefiles\3MCMC\WM164\7ReoxygenationAlpha\'];
filepath_load_Rd  = [newdir '\3MATLABsavefiles\3MCMC\WM164\8ReoxygenationRd\'];
...
load_alpha = load([filepath_load_alpha 'WM164_07_ReoxygenationAlpha.mat'],"-mat",'params_withnormalfit');
load_Rd = load([filepath_load_Rd 'WM164_08_ReoxygenationRd.mat'],"-mat",'params_withnormalfit');
...
k = k_unconverted; % [m^2 s^-1]
t_s=2;

modelfun = @(x,theta) function_reoxygenation_1simulation(theta,...
    k,...
    p_infinity,...
    Omega,...
    t_s,...
...
p_lower_bounds = [max(0.01,alpha_normoxia_lowerbound_from_Rc),...% theta(1) alpha_normoxia
    ...
    0,...% theta(16) = % nu
    max(0,NecroticRadiusInitial_normaldistribution_fit.mu-5*NecroticRadiusInitial_normaldistribution_fit.sigma)];  % theta(17) = necrotic_radii_at_ts
p_upper_bounds = [min(50,alpha_normoxia_upperbound_from_Rc),...% theta(1) alpha_normoxia
    ...
    OuterRadiusInitial_normaldistribution_fit.mu+3*OuterRadiusInitial_normaldistribution_fit.sigma,...% theta(15) = %outer_radii_at_ts
    1,... % theta(16) = % nu
    max(1,NecroticRadiusInitial_normaldistribution_fit.mu+5*NecroticRadiusInitial_normaldistribution_fit.sigma)];% theta(17) = necrotic_radii_at_ts
...
[ydata_mse] = function_reoxygenation_1simulation(p_mle,...
    k,...
    p_infinity,...
    Omega,...
    t_s,...
...
params = {
    ...
    {'necroticradii_at_ts',p_mle(17),p_lower_bounds(17), p_upper_bounds(17),NecroticRadiusInitial_normaldistribution_fit.mu,max(1,2*NecroticRadiusInitial_normaldistribution_fit.sigma)}
	};
...
modelfun_prediction_interval = @(x,theta) function_reoxygenation_1simulation(theta,...
    k,...
    p_infinity,...
    Omega,...
    t_s,...
...
close all
save([filepath_save 'MCMC_WM164_09_ReoxygenationAll' '.mat'],'-v7.3');
...
if include_plots == 1
    ...
    load([filepath_save 'MCMC_WM164_07_ReoxygenationAll' '.mat']);
    ...
    [ydata_globalmin] = function_reoxygenation_1simulation(p_mle,...
        k,...
        p_infinity,...
        Omega,...
        t_s,...
    ...
    [ydata_meanchain] = function_reoxygenation_1simulation(mean(chain),...
        k,...
        p_infinity,...
        Omega,...
        t_s,...
    ...
    fig_boxchart_predinterval = openfig([newdir '\4Figures\1FirstAnalysisExperimentalData\WM164\Scenario_4_OIN_1.fig']);
