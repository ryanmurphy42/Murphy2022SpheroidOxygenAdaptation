...
% create folders
cd ../
if ~exist('3MATLABsavefiles\3MCMC\WM983b\1Normoxia', 'dir')
    mkdir(['3MATLABsavefiles\3MCMC\WM983b\1Normoxia']);
end
if ~exist('4Figures\3MCMC\WM983b\1Normoxia', 'dir')
    mkdir(['4Figures\3MCMC\WM983b\1Normoxia']);
end


filepath_save = [newdir '\3MATLABsavefiles\3MCMC\WM983b\1Normoxia\'];
filepath_save_figs =  [newdir '\4Figures\3MCMC\WM983b\1Normoxia\'];

simulation_id = 'WM983b_01_NormoxiaGreenspan';
...
% remove outliers
WM983b_outer_tmp = load([filepath_load 'WM983b_confocal_scenario_1_outer.mat']);
WM983b_necrotic_tmp = load([filepath_load 'WM983b_confocal_scenario_1_necrotic.mat']);
WM983b_inhibited_tmp = load([filepath_load 'WM983b_confocal_scenario_1_inhibited.mat']);
...
num_spheroids = length(WM983b_outer.Radius);

Rcrit_estimate_per_spheroid = [];
Rcrit_estimate_counter = 1;
for i=1:num_spheroids
    if (WM983b_outer_tmp.Outlier(i)==0) && (WM983b_necrotic_tmp.Outlier(i)==0) % include if measurement is not an outlier
        OuterRadius_this_loop = WM983b_outer_tmp.Radius(i);
        NecroticRadius_this_loop = WM983b_necrotic_tmp.Radius(i);
...
for i=1:num_spheroids
    ...
    if (WM983b_outer_tmp.Outlier(i)==0) && (WM983b_necrotic_tmp.Outlier(i)==0) && (WM983b_inhibited_tmp.Outlier(i)==0) % include if measurement is not an outlier

        OuterRadius_this_loop = WM983b_outer_tmp.Radius(i);
        NecroticRadius_this_loop = WM983b_necrotic_tmp.Radius(i);
        InhibitedRadius_this_loop = WM983b_inhibited_tmp.Radius(i);
...
data.xdata = [WM983b_outer.Day;WM983b_necrotic.Day;WM983b_inhibited.Day]-day_shift; % Days
...
theta_first_guess =  [228.395229706770; % Rd
    0.411469948947368; % lambda
    0.233878767902795; % s
    247.481125049939;  % Rc
    212.169070125150]; % Roinit
...
p_upper_bounds =  [Rd_normaldistribution_fit.mu + 5*Rd_normaldistribution_fit.sigma; % theta(1) = Rd
    6; % theta(2) = lambda
    2; % theta(3) = s
    Rc_normaldistribution_fit.mu + 5*Rc_normaldistribution_fit.sigma;  % theta(4) = Rc
    OuterRadiusInitial_normaldistribution_fit.mu + 5*OuterRadiusInitial_normaldistribution_fit.sigma]; % theta(5) = Roinit
...
close all
save([filepath_save 'MCMC_WM983b_01_NormoxiaGreenspan' '.mat'],'-v7.3');
...
if include_plots == 1
    ...
    load([filepath_save 'MCMC_WM983b_01_NormoxiaGreenspan' '.mat']);
    ...
    fig_boxchart_predinterval = openfig([newdir '\4Figures\1FirstAnalysisExperimentalData\WM983b\Scenario_1_OIN_1.fig']);
