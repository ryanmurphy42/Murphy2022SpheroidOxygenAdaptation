...
% create folders
cd ../
if ~exist('3MATLABsavefiles\3MCMC\WM983b\3DeoxygenationAlpha', 'dir')
    mkdir(['3MATLABsavefiles\3MCMC\WM983b\3DeoxygenationAlpha']);
end
if ~exist('4Figures\3MCMC\WM983b\3DeoxygenationAlpha', 'dir')
    mkdir(['4Figures\3MCMC\WM983b\3DeoxygenationAlpha']);
end

filepath_save = [newdir '\3MATLABsavefiles\3MCMC\WM983b\3DeoxygenationAlpha\'];
filepath_save_figs =  [newdir '\4Figures\3MCMC\WM983b\3DeoxygenationAlpha\'];

simulation_id = 'WM983b_03_DeoxygenationAlpha';
...
deoxygenation_alpha_data= load([filepath_load 'WM983b_oxygenwasteanalysis_deoxy.mat']);

data.xdata = deoxygenation_alpha_data.confocal_WM983b.AllData.Day;
data.ydata = deoxygenation_alpha_data.alpha_vector*1e7;
...
theta_first_guess =  [alpha_initial_time_normaldistribution_fit.mu; % alpha_normoxia
    alpha_final_time_normaldistribution_fit.mu;  % alpha_hypoxia
    1]; % tau_alpha

fun_likelihood = @(p) ssfun(p,data);

p_first_guess = theta_first_guess;
p_lower_bounds =  [max(0, alpha_initial_time_normaldistribution_fit.mu - 5*alpha_initial_time_normaldistribution_fit.sigma); % alpha_normoxia
    max(0,alpha_final_time_normaldistribution_fit.mu - 5*alpha_final_time_normaldistribution_fit.sigma);  % alpha_hypoxia
    0]; % tau_alpha
p_upper_bounds =  [alpha_initial_time_normaldistribution_fit.mu + 5*alpha_initial_time_normaldistribution_fit.sigma; % alpha_normoxia
    alpha_final_time_normaldistribution_fit.mu + 5*alpha_final_time_normaldistribution_fit.sigma;  % alpha_hypoxia
    100]; % tau_alpha
...
params = {
    {'alpha normoxia', p_mle(1), p_lower_bounds(1),p_upper_bounds(1),alpha_initial_time_normaldistribution_fit.mu,3*alpha_initial_time_normaldistribution_fit.sigma}
    {'alpha hypoxia', p_mle(2), p_lower_bounds(2),p_upper_bounds(2),alpha_final_time_normaldistribution_fit.mu,3*alpha_final_time_normaldistribution_fit.sigma}
...
save([filepath_save 'WM983b_03_DeoxygenationAlpha' '.mat'],'-v7.3');
