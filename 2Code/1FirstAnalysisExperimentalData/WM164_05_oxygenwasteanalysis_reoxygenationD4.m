%% WM164 oxygen and waste analysis for reoxygenation D4 experiment
...
confocal_WM164.AllData = confocal_WM164.AllDatatmp1(sum(confocal_WM164.AllDatatmp1.Condition==[1,2,9,10],2)==1,:); % only include scenarios 3 (deoxygenation)
...
filepath_save_figs =  [newdir '\4Figures\1FirstAnalysisExperimentalData\WM164\ReoxyD4_'];
...
for i=1:number_of_spheroids
    ...
    if  sum(confocal_WM164.AllData.Condition(i) == [1,2])> 0
        % Deoxygenation - in hypoxia
        scenario_colour_vec(i) = 0;
    elseif sum(confocal_WM164.AllData.Condition(i) == [9,10]) > 0
        % Deoxygenation - in hypoxia
...
save([filepath_save 'WM164_oxygenwasteanalysis_reoxyD4.mat'],'-v7.3')
