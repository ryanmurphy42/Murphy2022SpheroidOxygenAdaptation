%% WM983B oxygen and waste analysis for deoxygenation experiment
...
confocal.AllData = readtable([newdir '\1Data\ExpOA_WM983b.xlsx'],'Sheet', 'Sheet1'); % load data
wm983b_data_include = (sum(cell2mat(confocal.AllData.CellLine) == 'WM983',2)==5); % identify WM983b data
confocal_WM983b.AllDatatmp2 = confocal.AllData(wm983b_data_include,:); % select WM983b data
confocal_WM983b.AllDatatmp1 = confocal_WM983b.AllDatatmp2((confocal_WM983b.AllDatatmp2.Keep==1),:); % only keep those that are not multiple spheroids or damaged spheroids
confocal_WM983b.AllData = confocal_WM983b.AllDatatmp1(sum(confocal_WM983b.AllDatatmp1.Condition==[17,11,12,13,14],2)==1,:); % only include scenarios 3 (deoxygenation)

% create folders
cd ../
if ~exist('3MATLABsavefiles\1FirstAnalysisExperimentalData\WM983b', 'dir')
    mkdir(['3MATLABsavefiles\1FirstAnalysisExperimentalData\WM983b']);
end
if ~exist('4Figures\1FirstAnalysisExperimentalData\WM983b', 'dir')
    mkdir(['4Figures\1FirstAnalysisExperimentalData\WM983b']);
end
if ~exist('4Figures\1FirstAnalysisExperimentalData\WM983b\OxygenProfiles', 'dir')
    mkdir(['4Figures\1FirstAnalysisExperimentalData\WM983b\OxygenProfiles']);
end

cd '2Code';
cd '1FirstAnalysisExperimentalData';

filepath_save = [newdir '\3MATLABsavefiles\1FirstAnalysisExperimentalData\WM983b\'];
filepath_save_figs =  [newdir '\4Figures\1FirstAnalysisExperimentalData\WM983b\Deoxy_'];
...
number_of_spheroids = size(confocal_WM983b.AllData,1);
...
for i=1:number_of_spheroids
    ...
    if  sum(confocal_WM983b.AllData.Condition(i) == [5,6,7,8,9,10,15,16,17,18,19])> 0
        % Measurement at 21% oxygen
        p_infinity = p_infinity_21;
    elseif sum(confocal_WM983b.AllData.Condition(i) == [1,2,3,4,11,12,13,14]) > 0
        % Measurement at 2% oxygen
        p_infinity = p_infinity_2;
    end

    if  sum(confocal_WM983b.AllData.Condition(i) == [11,12,13,14])> 0
        % Deoxygenation - in hypoxia
        scenario_colour_vec(i) = 0;
    elseif sum(confocal_WM983b.AllData.Condition(i) == [17]) > 0
        % Deoxygenation - in hypoxia
    ...
    OuterRadius_this_loop = confocal_WM983b.AllData.OuterRadius(i);
    NecroticRadius_this_loop = confocal_WM983b.AllData.NecroticRadius(i);
    PIMOuterRadius_this_loop = confocal_WM983b.AllData.PIMOuterRadius(i);
    InhibitedRadius_this_loop = confocal_WM983b.AllData.InhibitedRadius(i);
    ...
    if include_oxygen_profile_plots == 1
        ...
        ylim([0,25])
        xlim([0,500])
        saveas(gcf, [filepath_save_figs '\OxygenProfiles\' 'OxygenProfile_' num2str(i) '_' char(confocal_WM983b.AllData.fname(i)) '.fig'])
        saveas(gcf, [filepath_save_figs '\OxygenProfiles\' 'OxygenProfile_' num2str(i) '_' char(confocal_WM983b.AllData.fname(i)) '.png'])
        print([filepath_save_figs '\OxygenProfiles\' 'OxygenProfile_'  num2str(i) '_' char(confocal_WM983b.AllData.fname(i))],'-depsc2','-painters')
        close
    end
...
figure
scatter(1:number_of_spheroids,Rcrit_vector,25,confocal_WM983b.AllData.Condition,'filled')
xlabel('Spheroid')
ylabel('Rcrit')
ylim([0,400])
hold on
...
figure
scatter(confocal_WM983b.AllData.OuterRadius,Rcrit_vector,25,confocal_WM983b.AllData.Condition,'filled')
xlabel('Outer Radius')
ylabel('Rcrit')
xlim([0,400])
ylim([0,400])
...
figure
scatter(confocal_WM983b.AllData.Day,Rcrit_vector,25,scenario_colour_vec,'filled')
hold on
...
figure
scatter(1:number_of_spheroids,alpha_vector,25,confocal_WM983b.AllData.Condition,'filled')
xlabel('Spheroid')
ylabel('alpha')
ylim([0,2.5*10^-6])
box on
...
figure
scatter(confocal_WM983b.AllData.OuterRadius,alpha_vector,25,confocal_WM983b.AllData.Condition,'filled')
xlabel('Outer Radius')
ylabel('alpha')
xlim([0,400])
ylim([0,2.5*10^-6])
...
figure
scatter(confocal_WM983b.AllData.Day,alpha_vector,25,scenario_colour_vec,'filled')
xlabel('time [days]')
ylabel('alpha')
ylim([0,2.5*10^-6])
xlim([0,8.5])
...
figure
scatter(1:number_of_spheroids,p_n_vector,25,confocal_WM983b.AllData.Condition,'filled')
xlabel('Spheroid')
ylabel('pn')
box on
grid on
...
% Measured Rp(t) v Predicted Rp(t)
figure
hold on
scatter(confocal_WM983b.AllData.PIMOuterRadius(confocal_WM983b.AllData.PIMOuterRadius>0),PIMOuterRadius_prediction_vector(confocal_WM983b.AllData.PIMOuterRadius>0),25,scenario_colour_vec(confocal_WM983b.AllData.PIMOuterRadius>0),'filled')
...
mdl_PIMOuterRadius = fitlm(confocal_WM983b.AllData.PIMOuterRadius(confocal_WM983b.AllData.PIMOuterRadius>0),PIMOuterRadius_prediction_vector(confocal_WM983b.AllData.PIMOuterRadius>0));
figure
hold on
scatter(confocal_WM983b.AllData.PIMOuterRadius(confocal_WM983b.AllData.PIMOuterRadius>0),PIMOuterRadius_prediction_vector(confocal_WM983b.AllData.PIMOuterRadius>0),25,scenario_colour_vec(confocal_WM983b.AllData.PIMOuterRadius>0),'filled')
x_plot_OxygenAnalysis_Scenario_1_2_pim_7=0:400;
...
figure
hold on
scatter(1:number_of_spheroids,p_i_vector,25,confocal_WM983b.AllData.Condition,'filled')
...
figure
hold on
scatter(confocal_WM983b.AllData.OuterRadius,p_i_vector,25,confocal_WM983b.AllData.Condition,'filled')
plot([0,400],[21,21],'k--')
plot([0,400],[2,2],'k--')
xlabel('Outer Radius')
ylabel('pi')
...
figure
hold on
scatter(confocal_WM983b.AllData.Day,p_i_vector,25,scenario_colour_vec,'filled')
plot([0,8.5],[21,21],'k--')
plot([0,8.5],[2,2],'k--')
xlabel('time [days]')
ylabel('pi')
...
figure
hold on
scatter(confocal_WM983b.AllData.NecroticRadius,p_i_vector,25,scenario_colour_vec,'filled')
plot([0,400],[21,21],'k--')
plot([0,400],[2,2],'k--')
...
figure
hold on
scatter(confocal_WM983b.AllData.InhibitedRadius,p_i_vector,25,scenario_colour_vec,'filled')
plot([0,400],[21,21],'k--')
plot([0,400],[2,2],'k--')
...
figure
hold on
scatter(1:number_of_spheroids,Rd_vector,25,confocal_WM983b.AllData.Condition,'filled')
xlabel('Spheroid')
ylabel('Rd')
box on
grid on
...
figure
scatter(confocal_WM983b.AllData.OuterRadius,Rd_vector,25,confocal_WM983b.AllData.Condition,'filled')
xlabel('Outer Radius')
ylabel('Rd')
xlim([0,400])
ylim([0,400])
...
figure
scatter(confocal_WM983b.AllData.Day,Rd_vector,25,scenario_colour_vec,'filled')
xlabel('time [days]')
ylabel('Rd')
xlim([0,8.5])
ylim([0,400])
...
figure
hold on
scatter(confocal_WM983b.AllData.NecroticRadius,Rd_vector,25,scenario_colour_vec,'filled')
xlabel('Necrotic Radius')
ylabel('Rd')
xlim([0,400])
ylim([0,400])
...
figure
hold on
scatter(confocal_WM983b.AllData.InhibitedRadius,Rd_vector,25,scenario_colour_vec,'filled')
xlabel('Inhibited Radius')
ylabel('Rd')
xlim([0,400])
ylim([0,400])
...
save([filepath_save 'WM983b_oxygenwasteanalysis_deoxy.mat'],'-v7.3')
