function  [Radii,Ro_plot_times,Rn_plot_times,Ri_plot_times,Rnplus_plot_times] = function_reoxygenation_1simulation(theta,...
    k,...
    p_infinity,...
    Omega,...
    t_s,...
...
if length(theta) < 17
    ...
    necrotic_radii_at_ts = theta(17);
end


radii_at_ts = [outer_radii_at_ts % outer radius
    (4*pi/3).*necrotic_radii_at_ts.^3]; % necrotic core volume
...
if max(solution_function_Greenspan_reoxygenation_odes.x) >= max(t_span_normoxia)
    ...
    for i=1:length(plot_times)
        ...
        if variable_id_vec(i) == 3
            ...
            elseif y(2) < 0
                Ri_tmp =roots_Ri(logical((roots_Ri<y(1)).*(roots_Ri>0)));
            end
